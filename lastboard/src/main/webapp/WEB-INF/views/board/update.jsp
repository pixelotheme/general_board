...
// 			alert(decodeURI(result.replaceAll("+"," ")));
	...
	//=============================업로드된 파일 불러올경우 사용됨=================
	//파일명을 보여줄 함수
	function showPreUploadedFile(list){
...
// 				if(!obj.image){ 수정에서는 db에 저장된 정보를 가져와 비교해서 fileType으로 써야함 - 합치면 좋을텐데
				if(!obj.fileType){ 
...
// 				str += "<span class=\'deleteX\' data-file=\'"+fileCallPath+"\' data-type=\'"+(obj.image? "image":"file")+"\'>"; // x 태그 넣기
				str += "<span class=\'deleteX\' data-file=\'"+fileCallPath+"\' data-type=\'"+(obj.fileType? "image":"file")+"\'>"; // x 태그 넣기
				str += " x"
				str += "</span>";
				str	+= "</div>";
				str	+= "</li>";
...
// 					수정에서는 db에 저장된 정보를 가져와 비교해서 fileType으로 써야함 - 합치면 좋을텐데
				if(!obj.image){ 
// 				if(!obj.fileType){ 
					//이미지가 아닌경우 - 파일을 다운로드 할수있게 한다
...
<%-- 				<input type="hidden" name="word" value="${param.word}"> --%>
					...
					<div class="form-group">
						<label>글번호</label> <input class="form-control" name="no"
							value="${vo.no}" readonly="readonly" id="no" data-no="${vo.no}"/>
					</div>
					<div class="form-group">
					...
					<button type="submit" class="btn btn-default" data-oper="update">수정</button>
<!-- 					<button type="submit" class="btn btn-default" data-oper="remove">Remove</button> -->
					<button type="button"  data-oper="list" class="btn btn-default"
						onclick="location='view.do?no=${param.no }&perPageNum=${param.perPageNum}&page=${param.page }&key=${param.key }&word=${param.word }&inc=0'">취소</button>
				</form>
